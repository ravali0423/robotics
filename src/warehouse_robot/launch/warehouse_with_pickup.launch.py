#!/usr/bin/env python3
"""
Warehouse Robot with Package Pickup System

This launch file starts the warehouse simulation with:
- Robot spawning and control
- Package collision detection and attachment
...
def generate_launch_description():
    ...
    print("🎮 Web joystick controls available")
    print("📦 Package pickup system enabled")
    ...
    # Package collision detection and attachment system
    package_collision_handler = TimerAction(
        period=8.0,
        actions=[
            Node(
                package='warehouse_robot',
                executable='package_collision_handler.py',
                name='package_collision_handler',
                output='screen',
                parameters=[{
                    'robot_name': 'warehouse_car',
                    'package_name': 'pickup_package',
                    'attachment_height': 0.7
                }]
            )
        ]
    )
